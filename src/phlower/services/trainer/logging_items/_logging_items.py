...
class ILoggingItem(metaclass=abc.ABCMeta):
    @abc.abstractmethod
    def __init__(self, val: int, title: str = None) -> None: ...
    ...
    def format(
        ...
        *,
        ...
        padding_margin: int | None = None,
        key_orders: list[str] | None = None,
        title: str | None = None,
        **kwards,
    ) -> str: ...
...
class LoggingFloatItem(ILoggingItem):
        ...
        **kwards,
        ...
        if padding_margin is None:
            ...
            return self._format_padding(
                formatter=formatter, padding_margin=padding_margin, title=title
            )
    ...
    def _format_padding(
        self, formatter: str, padding_margin: int, title: str | None = None
    ) -> str:
        if title is None:
            title = self._title
...
class LoggingDictItem(ILoggingItem):
    def __init__(self, val: dict[str, float], title: str) -> None:
    ...
    def _format_padding(
        ...
        vals = [
            self._format_each_padding(
                k, self._val[k], formatter, padding_margin, title=title
            )
            for k in keys
        ]
        return "".join(vals)
...
class LoggingStrItem(ILoggingItem):
    ...
    def _format_padding(self, padding_margin: int, title: str = None) -> str:
        if title is None:
            title = self._title
        str_size = len(title) + padding_margin
