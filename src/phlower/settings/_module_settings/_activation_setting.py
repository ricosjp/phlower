...
class ActivationSetting(pydantic.BaseModel, IPhlowerLayerParameters):
    ...
    def gather_input_dims(self, *input_dims: int) -> int:
        if len(input_dims) != 1:
            raise ValueError(
                f"Activation takes only one input. Given: {input_dims}"
            )
    ...
    def check_n_nodes(self) -> Self:
        ...
        for i, v in enumerate(vals):
            ...
            raise ValueError(
                "nodes in Activation is inconsistent. "
