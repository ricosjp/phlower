...
class NaNToNumSetting(IPhlowerLayerParameters, pydantic.BaseModel):
    # This property only overwritten when resolving.
    nodes: list[int] | None = Field(None)

    nan: float = Field(0.0, frozen=True)
    posinf: float | None = Field(None, frozen=True)
    neginf: float | None = Field(None, frozen=True)
    ...
    def check_n_nodes(cls, vals: list[int]) -> list[int]:
        if len(vals) != 2:
            raise ValueError(
                f"size of nodes must be 2 in NaNToNumSetting. input: {vals}"
            )

        if vals[1] <= 0:
            raise ValueError(
                "nodes in NaNToNum is inconsistent. "
