...
class ConjugateGradientSolverSetting(
    ...
    matrix_name: str
    x0_name: str | None = Field(None, frozen=True)
    dirichlet_name: str | None = Field(None, frozen=True)
    rtol: float = Field(1e-5, frozen=True)
    ...
    def gather_input_dims(self, *input_dims: int) -> int:
        ...
        if self.x0_name is not None:
            expected_input_dims += 1
        if self.dirichlet_name is not None:
            expected_input_dims += 1
        if len(input_dims) != expected_input_dims:
            raise ValueError(
                "Invalid number of inputs to ConjugateGradientSolver. "
                f"input: {input_dims}"
            )

        if np.any(np.array(input_dims) != input_dims[0]):
            raise ValueError(
                "All inputs should have the same dimension. Given: {input_dims}"
            )
        return input_dims[0]
    ...
    def check_n_nodes(cls, vals: list[int] | None) -> list[int]:
        ...
        for i, v in enumerate(vals):
            ...
            raise ValueError(
                "Nodes in ConjugateGradientSolver is inconsistent. "
