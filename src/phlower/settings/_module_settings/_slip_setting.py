...
class SlipSetting(IPhlowerLayerParameters, pydantic.BaseModel):
    ...
    def gather_input_dims(self, *input_dims: int) -> int:
        if len(input_dims) != 3:
            raise ValueError("num of input should be 3 in Slip.")
    ...
    def confirm(self, self_module: IModuleSetting) -> None:
        ...
        for name in (self.normal_name, self.flag_name):
            if name not in input_keys:
                raise ValueError(
                    f"{name} is not found in input_keys. "
    ...
    def check_n_nodes(cls, vals: list[int]) -> list[int]:
        ...
        if len(vals) != 2:
            raise ValueError(
                f"Size of nodes must be 2 in SlipSettings. input: {vals}"
            )
        return vals
