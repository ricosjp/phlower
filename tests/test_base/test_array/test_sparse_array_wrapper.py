...
@pytest.mark.parametrize(
    "shapes, expected_shape", [([[(5, 6), (3, 5)], [(4, 9)]], (12, 20))]
)
def test__concatenate_nested_arrays(
    shapes: list[list[tuple[int]]], expected_shape
):
    rng = np.random.default_rng()
    ...
    for _shapes in shapes:
        ...
        sparse_arrays.append(concatenate(_arrs))
