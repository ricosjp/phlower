...
def test__fill_default_settings(nodes: list[int], kernel_sizes: list[str]):
    setting = TCNSetting(nodes=nodes, kernel_sizes=kernel_sizes)
...
def test__gather_input_dims(input_dims: list[int]):
    setting = TCNSetting(nodes=[10, 20], kernel_sizes=[2])
...
def test__raise_error_invalid_input_dims(input_dims: list[int]):
    setting = TCNSetting(nodes=[10, 20], kernel_sizes=[3])
...
_TEST_DATA_DIR = pathlib.Path(__file__).parent / "data/tcn_setting"


@pytest.mark.parametrize("yaml_file", ["check_tcn_nodes.yml"])
