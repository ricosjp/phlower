...
def test__can_accept_valid_n_nodes(nodes):
    _ = ConcatenatorSetting(nodes=nodes)


@pytest.mark.parametrize("nodes", [([5]), ([10, 10, 10])])
def test__raise_error_when_invalid_n_nodes(nodes):
...
def test__gather_input_dims(input_dims, desired):
    setting = ConcatenatorSetting()
...
_TEST_DATA_DIR = pathlib.Path(__file__).parent / "data/concatenator_setting"


@pytest.mark.parametrize("yaml_file", ["check_concatenator_nodes.yml"])
