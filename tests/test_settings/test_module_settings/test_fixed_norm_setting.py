...
def test__can_accept_valid_n_nodes(nodes: list[int] | None):
    _ = FixedNormSetting(nodes=nodes, mean_name="mean", std_name="std")
...
def test__raise_error_invalid_input_dims(input_dims: list[int]):
    setting = FixedNormSetting(mean_name="mean", std_name="std")
...
_TEST_DATA_DIR = pathlib.Path(__file__).parent / "data/fixed_norm_setting"


@pytest.mark.parametrize("yaml_file", ["check_fixed_norm_nodes.yml"])
