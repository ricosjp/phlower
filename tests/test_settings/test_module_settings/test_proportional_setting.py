...
@pytest.mark.parametrize("nodes", [([-1, 40]), ([10, 10])])
def test__can_accept_valid_n_nodes(nodes: list[int]):
    _ = ProportionalSetting(nodes=nodes)


@pytest.mark.parametrize("nodes", [([5]), ([])])
def test__raise_error_when_invalid_n_nodes(nodes: list[int]):
    with pytest.raises(ValueError) as ex:
        _ = ProportionalSetting(nodes=nodes)
    assert "size of nodes must be larger than 1" in str(ex.value)
...
def test__gather_input_dims(input_dims: list[int], desired: int):
    setting = ProportionalSetting(nodes=[-1, 10])
...
def test__nodes_is_updated_after_overwrite_nodes(
...
_TEST_DATA_DIR = pathlib.Path(__file__).parent / "data/proportional_setting"


@pytest.mark.parametrize("yaml_file", ["check_proportional_nodes.yml"])
