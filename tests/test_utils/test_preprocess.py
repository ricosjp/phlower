...
def test__dumped_numpy_scalar(val):
    actual = convert_to_dumped(val)
    if np.isnan(val):
...
def test_dumped_sequence_object(val):
    actual = convert_to_dumped(val)
    assert actual == val
