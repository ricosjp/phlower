...
def test__permutation_equivariance_for_vertexwise_tensor(
    ...
    _tensor = phlower_tensor(np.random.rand(*input_shape))
    n_nodes = _tensor.shape_pattern.get_n_vertices()
    permute_idxes = np.random.permutation(n_nodes)
    _permuted_tensor = _tensor[permute_idxes, ...]
    ...
    np.testing.assert_array_almost_equal(
        result1[permute_idxes].detach().numpy(),
...
@pytest.mark.parametrize(
    "input_shape",
    [(2, 10, 3, 1), (1, 20, 6), (5, 100, 5)],
)
def test__permutation_equivariance_for_time_series_tensor(
    nodes: list[int],
    activations: list[str],
    bias: bool,
    last_activation_name: str,
    ...
    nodes[0] = input_shape[-1]
    _tensor = phlower_tensor(np.random.rand(*input_shape), is_time_series=True)
    n_nodes = _tensor.shape_pattern.get_n_vertices()
    permute_idxes = np.random.permutation(n_nodes)
    _permuted_tensor = phlower_tensor(
        _tensor[:, permute_idxes, ...], is_time_series=True
    )
